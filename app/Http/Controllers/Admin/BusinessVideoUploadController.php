...
class BusinessVideoUploadController extends Controller
{
    //
    private $data = array(
        'route' => 'admin.business_video_upload.',
        'title' => 'Business Video',
        'menu' => 'business',
        'submenu' => 'business_video_upload',
    );
...
//        }
        ...
        if ($request->ajax()) {
            $records = BusinessVideoUpload::select('*')->with('videocategory')->orderBy('id','ASC');
        ...
        return view('admin.business_video_upload.index', $this->data);

    }
    public function create() {
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['category add']), 403);
        return view('admin.business_video_upload.create', $this->data);
    }
    public function store(Request $request) {
        ...
        $record = new BusinessVideoUpload($inputs);
        $record->save();
    ...
    public function show($id) {
        $this->data['record'] = BusinessVideoUpload::findOrFail($id);
        return view('admin.business_video_upload.show', $this->data);
    }
    ...
    public function update(Request $request, $id) {
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['category edit']), 403);
        $record = BusinessVideoUpload::findOrFail($id);
        /* Change Status Block */
        if ($request->ajax()) {
